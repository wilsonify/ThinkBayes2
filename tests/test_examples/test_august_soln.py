...
def test_birthday_problem():
    ...
    # Finds:
    #
    # The rate of claims-based ADHD diagnosis among children in states with a
    # September 1 cutoff was 85.1 per 10,000 children (309 cases among 36,319 children;
    # 95% confidence interval [CI], 75.6 to 94.2) among those born in August
    # and 63.6 per 10,000 children (225 cases among 35,353 children;
    # 95% CI, 55.4 to 71.9) among those born in September,
    # an absolute difference of 21.5 per 10,000 children (95% CI, 8.8 to 34.0);
    # the corresponding difference in states without the September 1 cutoff was
    # 8.9 per 10,000 children (95% CI, −14.9 to 20.8).
    # The rate of ADHD treatment was 52.9 per 10,000 children (192 of 36,319 children;
    # 95% CI, 45.4 to 60.3) among those born in August and 40.4 per 10,000 children
    # (143 of 35,353 children; 95% CI, 33.8 to 47.1) among those born in September,
    # an absolute difference of 12.5 per 10,000 children (95% CI, 2.43 to 22.4).
    #
    # These differences were not observed for other month-to-month comparisons,
    # nor were they observed in states with non-September cutoff dates for starting kindergarten.
    # In addition, in states with a September 1 cutoff, no significant differences between
    # August-born and September-born children were observed in rates of asthma, diabetes, or obesity.
    #
    ...
    # This might be explained by random variation, but it also might be due to parental manipulation;
    # if some parents hold back students born near the deadline,
    # the observations for these month would include a mixture of children who are relatively old for their grade,
    # and therefore less likely to be diagnosed.
    #
    # We could test this hypothesis by checking the actual ages of these students when they started school,
    ...
    # It seems like the lower rates in the last 3 months are unlikely to be due to random variation,
    # so it might be good to investigate the effect of "red shirting".
    #
    # But for now I will proceed with a linear logistic model.
    # The following table shows log odds of diagnosis for each month,
    # which I will use to lay out a grid for parameter estimation.
    ...
    # Most of these regression lines fall within the credible intervals of the observed rates,
    # so in that sense it seems like this model is not ruled out by the data.
    #
    # But it is clear that the lower rates in the last 3 months bring down the estimated slope,
    # so we should probably treat the estimated effect size as a lower bound.
    #
    # To express the results more clearly,
