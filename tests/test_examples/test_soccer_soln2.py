...
def test_soccer():
    # `Likelihood` computes the likelihood of `data` given `hypo`,
    # where `data` is an observed number of goals,
    # and `hypo` is a hypothetical goal-scoring rate in goals per game.
    # We can compute the likelihood of the data by evaluating the Poisson probability mass function (PMF).
    #
    ...
    # what should we believe about each team's goal scoring rate against each other?
    # We could use previous tournament results to construct the priors,
    # but to keep things simple,
    # I'll just use the average goal-scoring rate from all matches in the tournament,
    # which was 2.67 goals per game (total for both teams).
    #
    # To construct the prior,
    # I'll start with an unrealistic uniform distribution and update it with fake data
    # until the mean matches the observed rate for a single team, 1.34 goals per game.

    hypos = [_ for _ in linspace(start=0, stop=50, num=10)]
    suite = Soccer(hypos)
    assert sum(suite.Values()) == 250.0
    assert sum(suite.d.keys()) == 250.0
    assert sum(suite.d.values()) == pytest.approx(1, abs=0.001)
    assert suite.Total() == pytest.approx(1, abs=0.001)
    suite.Update(33)  # fake data chosen by trial and error to yield the observed prior mean
    suite.Mean()

    # According to this prior,
    # the goal-scoring rates are always greater than zero,
    # with the most likely value (a priori) near 0.5.
    # Goal scoring rates greater than 5 are considered unlikely.
    #
    ...
    # `Update` invokes the likelihood
    # function for each hypothetical value of $\lambda$ and updates the distribution accordingly.
    #
    # Since both teams scored fewer goals than the prior mean (1.4),
    # we expect both posterior means to be lower.
    # Germany's posterior mean is 1.2; Argentina's is 0.7.
    # We can plot the posteriors:

    # To answer the first question,
    # "How much evidence does this victory provide that Germany had the better team?",
    # we can compute the posterior probability that Germany had a higher goal-scoring rate:

    post_prob = germany.Mean() > argentina.Mean()
    print("posterior prob Germany > Argentina", post_prob)

    # Based on the prior distributions,
    # we would have said that Germany had a 50% chance of having the better team, or 1:1 odds.
    # Based on the posteriors, we would say that Germany has a 70% chance.
    # We can use the ratio of the prior and posterior odds to compute the Bayes factor,
    # which measures the strength of the evidence.
    ...
    #
    # If we knew the actual goal scoring rate,
    # $\lambda$, we could predict how many goals each team work score in a rematch.
    # The distribution of goals would be Poisson with parameter $\lambda t$.
    #
    # We don't actually know $\lambda$,
    # but we can use the posterior distribution of $\lambda$ to generate a
    # predictive distribution for the number of additional goals.
    ...
    # computes the predictive distribution of goals for each hypothesis,
    # and assembles a "meta-Pmf" which is a Pmf that maps from each predictive distribution to its probability.
    #
    # Finally, it uses `MakeMixture` to compute the mixture of the distributions.
    # Here's what the predictive distributions look like.

    # Using the predictive distributions, we can compute probabilities for the outcomes of a rematch.

    win = germany_pred.Mean() > argentina_pred.Mean()
    lose = germany_pred.Mean() < argentina_pred.Mean()
    tie = 1 - (win + lose)
    ...
    # If the score is tied after 90 minutes the teams play an additional 30 minute period.
    # We can use `PredictiveDist` again to compute the distribution of scores after 1/3 of a game:
    ...
    win = germany_pred_overtime.Mean() > argentina_pred_overtime.Mean()
    lose = germany_pred_overtime.Mean() < argentina_pred_overtime.Mean()
    tie = 1 - (win + lose)
