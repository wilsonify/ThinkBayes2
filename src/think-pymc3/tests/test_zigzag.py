...
def forward_hierarchical(size=1):
    alpha = expon().rvs(size=size)
    beta = expon().rvs(size=size)
    mu = gamma(a=alpha, scale=1 / beta).rvs(size=size)
    goals = poisson(mu).rvs(size=size)
    return goals[0]
...
def test_hockey():
    ...
    # If we simulate 1000 games, we can see what the distribution looks like.
    # The average of this sample should be close to `lam_per_game`.
    ...
    # I'll start with a probability mass function (PMF), which I'll implement using a `Counter`.
    #
    ...
    #
    # For the simulation we just did,
    # we can figure out the distribution analytically:
    # it's a binomial distribution with parameters `n` and `p`, where `n` is the
    # number of minutes and `p` is the probability of scoring a goal during any minute.
    #
    ...
    # But plotting PMFs is a bad way to compare distributions.
    # It's better to use the cumulative distribution function (CDF).
    ...
    # the binomial distribution converges to the Poisson distribution with parameter
    # `mu = n * p`, which is also `mu = lam_per_game`.
...
@pytest.mark.skip(reason='version compatibiliity issues')
def test_hockey_pymc():
    # ## Warming up PyMC
    #
    # Soon we will want to use `pymc3` to do inference,
    # which is really what it's for.
    # But just to get warmed up,
    # I will use it to generate a sample from a Poisson distribution.
    # And simulate 10 games.
    lam_per_game = 2.7
    mu = lam_per_game
    min_per_game = 60
    ...
    #
    # One of the nice things about the Poisson distribution is that we can compute its CDF and PMF analytically.
    # We can use the CDF to check, one more time, the previous results.

    xs = np.arange(11)
    ps = poisson.cdf(xs, mu)

    plot_cdfs(sample_sim, sample_bin, sample_poisson, sample_pm)
    plt.plot(xs, ps, label="analytic", linestyle="dashed")
    legend()

    # And we can use the PMF to compute the probability of any given outcome.
    # Here's what the analytic PMF looks like:
    ...
    # And here's a function that compute the probability of scoring a given number of goals in a game,
    # for a known value of `mu`.
    ...
    # This function also works with a sequence of goals,
    # so we can compute the probability of scoring 6 goals in the first game and 3 in the second.

    poisson_likelihood(goals=[6, 2], mu=2.7)


def test_hockey_inference():
    # ## Bayesian inference with grid approximation
    #
    # Ok, it's finally time to do some inference!
    # The function we just wrote computes the likelihood of the data,
    # given a hypothetical value of `mu`:
    #
    # $\mathrm{Prob}~(x ~|~ \mu)$
    #
    ...
    # I'll start with a uniform prior just to keep things simple.
    # We'll choose a better prior later.
    ...
    # the posterior is the likelihood function,
    # and the MAP is the value of `mu` that maximizes likelihood,
    # which is the observed number of goals, 6.
    #
    ...
    # I'll use scores from previous Stanley Cup finals to estimate the parameters of a gamma distribution.
    #
    # Why gamma?  You'll see.
    #
    # Here are (total goals)/(number of games) for both teams from 2013 to 2017,
    # not including games that went into overtime.

    xs = [13 / 6, 19 / 6, 8 / 4, 1.0, 10 / 6, 13 / 6, 1.0, 4 / 2, 5 / 3, 6 / 3]
    ...
    # Suppose the same team plays again and scores 2 goals in the second game.
    # We can perform a second update using the posterior from the first update as the prior for the second.
    ...
    #
    # I'm using a gamma distribution as a prior in part
    # because it has a shape that seems credible based on what I know about hockey.
    #
    # But it is also useful because it happens to be the conjugate prior of the Poisson distribution,
    # which means that if the prior is gamma and we update with a Poisson likelihood function,
    # the posterior is also gamma.
    #
    # See https://en.wikipedia.org/wiki/Conjugate_prior#Discrete_distributions
    #
    # And often we can compute the parameters of the posterior with very little computation.
    # If we observe `x` goals in `1` game, the new parameters are `alpha+x` and `beta+1`.
    ...
    # to generate a posterior prediction by taking the mean of the posterior distribution
    # and drawing samples from `Poisson(mu)` with just a single value of `mu`.
    #
...
@pytest.mark.skip(reason="version issues")
def test_hockey_prior_predictive():
    ...
    # This might not be a sensible way to use PyMC.
    # If we just want to sample from the prior predictive distribution,
    # we could use NumPy or SciPy just as well.
    # We're doing this to develop and test the model incrementally.
    #
    ...
    # Now let's extend the model to sample from the prior predictive distribution.
    # This is still a silly way to do it, but it is one more step toward inference.
    ...
    # Let's see how the results compare with a sample from the prior predictive distribution,
    # generated by plain old NumPy.
    ...
    # With `goals` fixed, the only unknown is `mu`, so `trace` contains a sample drawn
    # from the posterior distribution of `mu`.  We can plot the posterior using a function provided by PyMC:
    ...
    # ## Going hierarchical
    #
    # So far, all of this is based on a gamma prior.  To choose the parameters of the prior,
    # I used data from previous Stanley Cup finals and computed a maximum likelihood estimate (MLE).
    ...
    #
    # In other words, I have ignored two important sources of uncertainty.
    # As a result, my predictions are almost certainly too confident.
    #
    # The solution is a hierarchical model, where `alpha` and `beta` are the parameters that control
    # `mu` and `mu` is the parameter that controls `goals`.
    # Then we can use observed `goals` to update the distributions of all three unknown parameters.
    #
    # Of course, now we need a prior distribution for `alpha` and `beta`.
    # A common choice is the half Cauchy distribution
    # (see [Gelman](http://www.stat.columbia.edu/~gelman/research/published/taumain.pdf)),
    # but on advice of counsel, I'm going with exponential.
    ...
    # This distribution represents radical uncertainty about the value of this distribution:
    # it's probably between 0.1 and 10, but it could be really big or really small.
    #
    ...
    # In effect, the model is saying "I have never seen a hockey game before.
    # As far as I know, it could be soccer, could be basketball, could be pinball."
    #
    # If we zoom in on the range 0 to 10,
    # we can compare the prior implied by the hierarchical model with the gamma prior I hand picked.
    ...
    # They agree that the most likely values are less than 10,
    # but the hierarchical model admits the possibility that `mu` could be orders of magnitude bigger.
    #
    ...
    # The posterior mean is close to the observed value,
    # which is what we expect with a weakly informative prior.
    ...
    # On the basis of one game (and never having seen a previous game),
    # here's the probability that Vegas is the better team.

    np.mean(sample_post_mu_VGK > sample_post_mu_WSH)

    # ## More background
    #
    # But let's take advantage of more information.
    # Here are the results from the five most recent Stanley Cup finals,
    # ignoring games that went into overtime.
    ...
    # They are lower with the background information than without,
    # and closer together.
    # Here's the updated chance that Vegas is the better team.
    ...
    #
    # In a Poisson process with rate parameter `mu`,
    # the time until the next event is exponential with parameter `lam = 1/mu`.
    #
