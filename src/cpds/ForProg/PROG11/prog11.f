...
C COR = CORRELATION COEFFICIENT
        ...
        OPEN(UNIT=3, FILE='DATA11.DAT', STATUS='OLD' , ERR=I8)

        OPEN(UNIT=I,FILE='PRN')

        READ(3, *, ERR=I9)N
        READ(3, *, ERR=I9)(X(I), Y(I), I=1, N)
        ...
        WRITE(1, 100)
        FORMAT(//,1H0,25X,'CURVE FITTING FOR TWO VARIABLES' , /1H ,78(1H*))

        WRITE(1, 110)
        FORMAT(//,1HO,25X,'X',15X,'Y',/1H,78(1H-))

        DO 10 I=1, N

        WRITE(1, 120)I, X(I), Y(I)
        FORMAT(1H0, 4X, I4, 6X, F14.3, 6X, EI4.6)
        CONTINUE
        ...
        WRITE(1 , 130)
        FORMAT(//, 1H  ,25X, 'THE RESULTS ARE: ' ,/1H ,78(1H*) )

        WRITE(1, 140)
        FORMAT(/,IOX,'EQUATION',IOX,'REGRESSION COEFFICIENTS', 5X,'CORRELATION')
        
        WRITE( 1 , 150)
        FORMAT(28X, 'INTECEPT: A' , 5X, 'SLOPE: B' , 4X, ' COEFFICIENT ' , /1H , 78( 1H*) )
...
C        CURVE FITTING FOR : Y=A+BX
        ...
        CALL LINFIT(X,Y, N, A1,B1, COR1)
        
        WRITE(1, 160)A1, B1, COR1
        FORMAT(2X, '1.', 5X, 'Y=A+BX', IOX, 3(E10.4,5X))
...
C CURVE FITTING FOR Y=A+BX^2
        ...
        FORMAT (2X,'2.',5X,'Y=A+B*X^2',7X,3(E10.4,5X))
...
C CURVE FITTING FOR Y=A+B.X^0.5
        ...
        FORMAT(2X,'3.',5X,'Y=A+B*X^0.5',5X,3(E10.4,5X))
...
C LINEARIZE TO Ln(Y)=Ln(A)+BX
        ...
        FORMAT(2X,'4.',5X,'Y=A*EXP(B*X)',4X,3(E10.4,5X))
...
C CURVE FITTING FOR Y=A+B.Ln(X)
        ...
        FORMAT(2X,'5.',5X,'Y=A+B*Ln(X)',5X,3(E10.4,5X))
...
C CURVE FITTING FOR Y = A + B . I / X
        ...
        FORMAT(2X,'7.',5X,'Y=A+B*1/X',7X,3(E10.4,5X))
...
C CURVE FITTING FOR I / Y = A + B . I / X
        ...
        FORMAT(2X,'8.',5X,'1/Y=A+B*1/X',5X,3(E10.4,5X))
        ...
        CALL COMP(X, Y, N, COR1, COR2, COR3, COR4, COR5, COR6, COR7, COR8)
        CLOSE(3, STATUS = 'KEEP' )
        CLOSE(1)
        STOP
        END
...
C THIS PRINTS THE RESULTS ON THE SCREEN.
        ...
        FORMAT(6X,'XMEAN:',F12.3,3X,'YMEAN:',F12.4)
...
C CALCULATE THE CORRELATION COEFFICIENT
        ...
        WRITE(*,60)A, B, COR
        FORMAT(6X,'A=',F12.3,3X,'B=',F12.3,3X,'COR=',F12.4)
        RETURN
        END
...
C FIND THE MAXIMUM VALUE OF THE CORRELATION COEFFICIENTS
        ...
        WRITE(1100)CORR
        FORMAT(//,2X,'MAXIMUM CORRELATION REGRESSION ANALYSES. COEFFICIENT:',F12.4)
...
C THE CONSTANTS AND THE CORRELATION COEFFICIENT.
        ...
        WRITE(1, 100)
        FORMAT(/,1H0, 21X,'X-ACTUAL',13X,'Y-ACTUAL', 11X, 'Y-ESTIMATE', /,1H ,78(1H-))
        DO 10 I = 1,N
        ...
        WRITE (1 130)A, B, COR
        FORMAT( / / , 1H0,6X,'CONSTANTS FOR THE EQUATION:',/1H0,6X, 'A:', 6X, F12.4, /1H0, 6X, 'B:', 6X, F12.4,/1H0,'CORRELATION COEFFICIENT:',FI2.4/ ,1H ,78(1H-))
