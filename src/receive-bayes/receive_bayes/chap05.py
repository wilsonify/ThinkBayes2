...
def cookie_strategy(self, body: dict):
    ...
    likelihood_ratio = 0.25 / 0.5  # If we draw another cookie, and it's chocolate.
    post_odds *= likelihood_ratio  # update
    logging.info("%r", f"post_odds = {post_odds}")
    post_prob = Probability(post_odds)
    logging.info("%r", f"post_prob = {post_prob}")
    self.publish(post_prob)
...
def oliver_blood_strategy(self, body: dict):
    ...
    """
    like1 = 0.01  # type 'AB' blood
    like2 = 2 * 0.6 * 0.01  # type 'O' blood
    likelihood_ratio = like1 / like2
    logging.info("%r", f"likelihood_ratio = {likelihood_ratio}")
    favor_against = "against" if likelihood_ratio < 1 else "in-favor"  # Since the ratio is less than 1,
    ...
    post_odds = prior_odds * like1 / like2  # the posterior odds would be 0.83
    Probability(post_odds)  # which corresponds to a probability
