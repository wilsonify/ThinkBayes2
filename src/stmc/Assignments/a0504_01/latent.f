...
C Analysis program for multicanonical data.
     ...
     & nla_in(nd),acta(0:mlink), ha_ar(nrpt,0:mlink),dha(nrpt),dla(nrpt)
...
C Read betas
      open(iud2,file="beta_ar.d", form="formatted",status="old")
C
...
c        print*, "----------- beta=", beta0, "-------------"
     ...
     &                          (ph(ilink)-10**5*ha(ilink))**2
      ...
      call stebj0(nrpt,dla,dlm,dlv,dle)
      write(iuo, '("latent heat: ",F8.5," +/-",F8.5)') dlm, dle
C
      close (iud1)
C
      write(iuo,*) "All done."
