...
C Potts muca with Metropolis updating, time series version.
      ...
      dimension tsa(nmeas,0:nq),nstate(0:nqm1)
      include 'lat.dat'
      data ltun0/.false./
      ltest=.true.
      ltest=.false.
     ...
     &  mu_swp,ntun,nequi,nrpt,nmeas
        ...
        do irpt=1,nrpt
          ...
          do imeas=1,nmeas
            ...
            call potts_mag(ns,nqm1,ista,nstate)
            do iq=0,nqm1 ! Unnormalized Potts magnetization.
              tsa(imeas,iq)=nstate(iq)*one
            end do
            tsa(imeas,nq)=ONE*iact ! Unnormalized action variable.
          end do
          acpt=acpt/(nmeas*ns)
          call write_progress(iud2,"irpt,ntu_m,acpt:",irpt,ntu_m,acpt)
          write(iuo,'(" irpt,ntu_m,acpt:",2I10,F9.3)') irpt,ntu_m,acpt
          write(iud1) ha,tsa,acpt,irpt,ntu_m
        end do
      end if
C
      stop
