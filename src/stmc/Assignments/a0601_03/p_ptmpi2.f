...
C Parallel tempering MPI implementation with recursion pt_rec2.f.
...
C
     ...
     &         form="formatted",status="old",access="append")
        WRITE(IUD,*) "p_ptmpi2.f with recursion pt_rec2.f."
        CLOSE(IUD)
      END IF 
C
      do iequi=1,nequi
...
C
     ...
     &                NACPT_PT(0),1,MPI_INTEGER,MPI_COMM_WORLD,IERROR)
        CALL 
     &    PT_REC2(BA,BASUM,NACPT_PT,IP_B,IPI_B,MY_ID,NPM1,NPT_REC,IUD)
