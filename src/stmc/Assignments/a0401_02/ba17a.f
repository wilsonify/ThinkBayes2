      PROGRAM BA17a ! Variant of BAUTO
C
...
C
     ...
     &              9X,"RATIO")')
      OPEN(UNIT=IUD,FILE='b_aint17a.d',
...
C Bounds corresponding to the one sigma levlel:
        RAT_U=RATIO+HALF*(RAT_U-RATIO)
        RAT_D=RATIO+HALF*(RAT_D-RATIO)
        XBIN=ONE*(NBIN-1)
        WRITE(IUD,'(I4,F8.2,I10,3F10.5)') KB,(3*XBIN+30),NBINS,
     &    (0.3+0.8*RATIO),(0.3+0.8*RAT_U),(0.3+0.8*RAT_D)
      END DO
      CLOSE(IUD)
C
      STOP
