...
C
     ...
     &        NBOUNCE(MY_B_TS(itime,MY_ID))+1
            END IF ! MY_B(1,MY_I) is assigned next.
            MY_B_TS(1,MY_ID)=MY_B_TS(itime,MY_ID) ! itime-1 in IF.
          END IF
        END DO
      END DO
      write(iuo,'(" MY Beta:    ",8I5)') (MY_B,MY_B=0,NPM1) 
      write(iuo,'(" Accepted:   ",8I5)') (NACPT(MY_B),MY_B=0,NPM1) 
      write(iuo,'(" Bounced:    ",8I5)') (NBOUNCE(MY_B),MY_B=0,NPM1) 
      ASUM_PT=ZERO
      DO MY_B=0,NPM1
        ACPT_PT(MY_B)=(THREE*NACPT(MY_B))/(2*ntm1)
        ASUM_PT=ASUM_PT+ACPT_PT(MY_B)
        ACPT_B(MY_B)=(ONE*NBOUNCE(MY_B))/(ONE*NACPT(MY_B))
      END DO
      write(iuo,'(" Bounce Prob:",8F5.2)') (ACPT_B(MY_B),MY_B=0,NPM1) 
      write(iuo,'(" Accept Prob:",8F5.2)') (ACPT_PT(MY_B),MY_B=0,NPM1) 
      write(iuo,'(" ASUM_PT =",G16.7,
     &  " (slightly different counting than before)")') ASUM_PT
C
