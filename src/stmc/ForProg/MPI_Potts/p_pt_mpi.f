...
C
      IF(MY_IND1.EQ.INDEX) THEN ! Processes MY_IND1.
        NDEST3L=NEIGH(1)
        NSEND3L=-2
        IF(NEIGH(2).LT.N_PROC) THEN
          NSEND(1)=NEIGH(1)
          NSEND(2)=iact
          NSEND(3)=IACPT_PT
     ...
     &                  MPI_COMM_WORLD,ISTATUS,IERR)
          ...
          END IF
        END IF
        IF(NDEST3L.GE.0) CALL MPI_SEND(NSEND3L,1,MPI_INTEGER,NDEST3L,
     &                                     itag,MPI_COMM_WORLD,IERR)
      END IF
C
      IF(MY_IND2.EQ.INDEX) THEN ! Processes MY_IND2.
        NDEST3R=NEIGH(2)
        NSEND3R=-2
        IF(NEIGH(1).GE.0) THEN
     ...
     &                  MPI_COMM_WORLD,IERR)
        END IF
        IF(NDEST3R.LT.N_PROC) CALL MPI_SEND(NSEND3R,1,MPI_INTEGER,
     &                              NDEST3R,itag,MPI_COMM_WORLD,IERR)
      END IF
C
      IF(MY_IND3.EQ.INDEX) THEN ! Processes MY_IND3.
        IF(NEIGH(1).GE.0) THEN
          CALL MPI_RECV(NRECV3R,1,MPI_INTEGER,NEIGH(1),
     &                  itag,MPI_COMM_WORLD,ISTATUS,IERR)
           IF(NRECV3R.NE.-2) NEIGH(1)=NRECV3R
        END IF 
        IF(NEIGH(2).LT.N_PROC) THEN
          CALL MPI_RECV(NRECV3L,1,MPI_INTEGER,NEIGH(2),
     &                  itag,MPI_COMM_WORLD,ISTATUS,IERR)
          IF(NRECV3L.NE.-2) NEIGH(2)=NRECV3L
        END IF 
      END IF
C
      RETURN
