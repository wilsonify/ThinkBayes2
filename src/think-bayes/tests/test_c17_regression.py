...
def test_create_pmf():
    ...
    sample_data["YEAR"] = sample_data["DATE"].dt.year
    snow = sample_data.groupby(sample_data["YEAR"])["SNOW"].sum()
    pmf = create_pmf(snow)
    assert isinstance(pmf, Pmf)
    assert np.isclose(pmf.mean(), snow.mean())
    # Compute the weighted standard deviation for the PMF
    weights = pmf.ps
    values = pmf.qs
    pmf_weighted_std = np.sqrt(np.sum(weights * (values - pmf.mean()) ** 2))
    assert np.isclose(pmf_weighted_std, snow.std(), atol=2)
...
def test_compute_likelihood():
    ...
    ys = snow["SNOW"]
    prior_slope, prior_intercept, prior_sigma = create_priors()
    likelihood = compute_likelihood(xs, ys, prior_slope, prior_intercept, prior_sigma)
