...
from thinkbayes import Suite, Pmf, Cdf
...
def UniformPrior2(label="uniform"):
    """Makes a Suite with a uniform prior."""
    suite = Euro3(range(0, 101), label=label)
    return suite


def TrianglePrior2(label="triangle"):
    """Makes a Suite with a triangle prior."""
    suite = Euro3(label=label)
    for x in range(0, 51):
        suite[x] = x
    for x in range(51, 101):
        suite[x] = 100 - x
...
def RunUpdate2(suite, heads=140, tails=110):
    """Updates the Suite with the given number of heads and tails.
